...
import {
  Search, Refresh, View, Edit, Plus, Check, Lock, Unlock, OfficeBuilding, Shop, CircleCheck, SuccessFilled, Warning
...
const loadEnterpriseList = async () => {
  ...
  try {
    ...
    const inactive = enterpriseList.value.filter(item => item.status === 'inactive').length

    statistics.value = { total, verified, active, inactive }
...
.stat-item.verified .stat-icon { background: #67c23a; }
.stat-item.active .stat-icon { background: #67c23a; }
.stat-item.inactive .stat-icon { background: #e6a23c; }
